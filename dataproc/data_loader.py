...
from ..utils.tar_handling import (entry_size, parse_header,
                                  uncompressed_tarfile_size, unpack_file)
...
class DataLoader(Base):
    def __init__(
        ...
        **kwargs,
        ...
        if data_cache is None:
            data_cache = Path().cwd()
        if cluster is None:
            self.cluster = LocalCluster(n_workers=(psutil.cpu_count()-1*2), threads_per_worker=1)
    ...
    def read_layers(
        ...
        for p in sorted(data_fs.glob("*.pcd", detail=False), key=lambda x: float(x.split("/")[-1].split(".")[0])):
            ...
            try:
                ...
                mem_limit = psutil.virtual_memory().total // \
                    (2 * self.cluster._threads_per_worker() * nworkers) # aim to use half ram per worker
            assert (
                file_size < (mem_limit)
        ...
        for i, file_list in enumerate(self.progressbar(batches, position=2)):
            ...
            local_file_list = [
                f"{read_arr_cache}{x}" for x in 
                (x if x[1] == "/" else f"/{x}" for x in file_list)
            ]
            data_fs.get(file_list, local_file_list) # maybe making this async would speed up process?
            data = read_selected_layers(local_file_list)
            darr = da.from_array(
                data,
                chunks=(
                    (*tuple(repeat(chunk_size, data.shape[0] // chunk_size)), data.shape[0] % chunk_size),
        ...
        target_options = self.fs.storage_options.get(
            "target_options",
            self.fs.storage_options
        )
        ddf.to_parquet(
    ...
    # return metadata for the cache. Caches last metadata result based on checksum of entire cache folder
    @property
    ...
    def save(self, filepath: Path | str = Path("data")):
        ...
        if "views" in attrs: del attrs["views"]
    ...
    def _unpack_savefile(self, filepath: str, blocksize: int = 512):
        ...
        with self.fs.open(filepath, mode="rb", compression="gzip") as t:
            while header := t.read(blocksize):
                ...
                size = tree_metadata[f"{self.fs.info(self._data_cache)['name'][:-5]}{entry_name}"]["size"]  # TEMP PATCH
