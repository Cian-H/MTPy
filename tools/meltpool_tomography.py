...
# mtpy_defaults = {"data_path": Path.cwd()}
# sample_defaults = {"w": ["t1", "t2"], "plotparams": {}}
# viz_plot_defaults = {"colormap": "plasma", "colorbar": True, "shape": 2046}
# viz_figure_defaults = {"facecolor": "white"}
# mpl_plot_defaults = {
#     "cmap": "plasma",
#     "marker": ".",
# }
# mpl_figure_defaults = {"facecolor": "white"}
...
class MeltpoolTomography(DataProcessor, DataPlotter):
    ...
    #         if not ((type(vmin) is dict) and (type(vmin) is dict)):
    ...
    #             if plotparams["colorbar"] and "colorbar_label" not in plotparams:
    #                 plotparams[
    #                     "colorbar_label"
    #                 ] = f"{self.temp_label} ({self.temp_units})"
    #             # Create plot
    #             heatmap = layer_data.viz.heatmap(
    #                 x="x", y="y", what=f"max({column})", **plotparams
    #             )
    #             # Get figure and axis
    ...
    #                 samples_present = {
    #                     key: val
    #                     for (key, val) in enumerate(self.sample_labels)
    #                     if key in samples_present
    #                 }
    #                 for i, xy in samples_present.items():
    #                     ax.annotate(str(i), xy)
    #             # Save figure and clear pyplot buffer
    #             fig.savefig(
    #                 f"{output_path}/{column}_heatmap_z={layer}.{filetype}",  # noqa
    #                 **figureparams,
    #             )
    ...
    # def samples_to_heatmap(self, output_path, global_norm: bool = False, **kwargs):
    ...
    #         self.layers_to_heatmap(f"{output_path}/{sample_number}", **kwargs)
    #     self._quiet_callback = False

    #     self._qprint("Sample heatmaps complete!\n")

    # def layers_to_scatter(
    #     self,
    #     output_path,
    #     filetype: str = "png",
    #     sample_labels: bool = True,
    ...
    #             plotparams["norm"] = plt.Normalize(vmin=local_vmin, vmax=local_vmax)
    #             mappable = plt.cm.ScalarMappable(
    #                 norm=plotparams["norm"], cmap=plotparams["cmap"]
    #             )
    #             # Filter out target layer
    ...
    #             # Save figure and clear pyplot buffer
    #             fig.savefig(
    #                 f"{output_path}/{column}_scatter_z={layer}.{filetype}",  # noqa
    #                 **figureparams,
    #             )
    #             # Clear figure and ensure large, local objects get deleted
    #             fig.clf()
    ...
    # def samples_to_scatter(self, output_path, global_norm: bool = False, **kwargs):
    #     # apply defaults to kwargs
    #     kwargs = apply_defaults(kwargs, sample_defaults)
    ...
    #     # Prep progress bar iterator (assigned to variable for code clarity)
    #     progbar_iterator = self.progressbar(
    #         samples, total=len(samples), desc="Samples", position=0, disable=self.quiet
    #     )
    ...
    #         kwargs["sample_filter"] = lambda x: x == sample_number
    #         # Plot all layers in the sample
    #         self.layers_to_scatter(f"{output_path}/{sample_number}", **kwargs)
    #     self._quiet_callback = False

    #     self._qprint("Sample scatters complete!\n")

    # def layers_to_3dscatter(
    #     self,
    #     output_path,
    #     filetype: str = "png",
    #     colorbar: bool = True,
    ...
    #     if not self._quiet_callback:
    #         self._qprint(
    #             f"\nPreparing to generate 3dscatter in {output_path}..."
    #         )  # noqa
    #     # Prepare path for figure generation
    #     output_path = str(Path(output_path).expanduser())
    #     Path(output_path).mkdir(parents=True, exist_ok=True)
    ...
    #         mappable = plt.cm.ScalarMappable(norm, cmap=plotparams["cmap"])
    ...
    #         if colorbar:
    #             plt.colorbar(mappable, label=f"{self.temp_label} ({self.temp_units})")
    #         # Finally, format plots appropriately
    #         ax.set_xlabel("x")
    #         ax.set_ylabel("y")
    #         ax.set_zlabel("z")
    ...
    # #   of entire layer
    # def samples_to_3dscatter(self, output_path, global_norm: bool = False, **kwargs):
    ...
    #         kwargs["sample_filter"] = lambda x: x == sample_number
    #         # Plot all layers in the sample
    #         self.layers_to_3dscatter(f"{output_path}/{sample_number}", **kwargs)
    #     self._quiet_callback = False

    #     self._qprint("Sample 3dscatters complete!\n")

    # def layers_to_3dscatter_interactive(
    #     self,
    #     output_path,
    #     downsampling: int = 1,
    ...
    #     if not self._quiet_callback:
    #         self._qprint(
    #             f"\nPreparing to generate interactive 3dscatter in {output_path}..."
    #         )  # noqa
    ...
    #         plotparams["range_z"] = [0.0, data["z"].max() + 0.1]
    #     # add n to hover data
    #     hover_data += ["n"]
    ...
    #         )
    #         fig.write_html(
    #             f"{output_path}/3dscatter_interactive_{column}.html", **figureparams
    #         )
    #     # Delete last filter to remove unneeded vaex filters
    #     del data
    ...
    # def samples_to_3dscatter_interactive(
    #     self, output_path, global_norm: bool = False, **kwargs
    # ):
    #     "Generates interactive 3d figures for every labelled sample"
    #     self._qprint(
    #         f"\nPreparing to generate sample interactive 3dplots in {output_path}..."
    #     )  # noqa
    ...
    #             f"{output_path}/{sample_number}", **kwargs
    #         )
    #     self._quiet_callback = False
    ...
    #     if samples:
    #         self.calculate_stats(groupby="sample").export_csv(
    #             f"{output_path}sampletemps.csv"
    #         )
    #     if sample_layers:
    #         self.calculate_stats(groupby=["sample", "z"]).export_csv(
    #             f"{output_path}samplelayertemps.csv"
    #         )
